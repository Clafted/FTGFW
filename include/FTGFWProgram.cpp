...
int FTGFWProgram::initProgram(int screenWidth, int screenHeight, const char* windowName) {
	...
	Scene::window = window;
	Scene::componentManager = &componentManager;
	...
	systemManager.registerSystem(std::make_shared<KinematicSystem>());
	systemManager.registerSystem(std::make_shared<CameraSystem>());
	systemManager.registerSystem(std::make_shared<CollisionSystem>());
...
int FTGFWProgram::initRenderLoop(std::string startingScene, std::string vertexShaderPath, std::string fragmentShaderPath) {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glfwPollEvents();
		// Update all Systems
		systemManager.update(window, (float) deltaTime);
		frameCount++;
	}
