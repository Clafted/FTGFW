...
RenderComponent Model::processMesh(aiMesh* mesh, const aiScene* scene) {
	// Process vertices
	...
	return RenderComponent(vertices, indices, textures);
}
...
std::vector<Texture> Model::loadMaterialTextures(aiMaterial* mat, aiTextureType type) {
	...
	for (unsigned int i = 0; i < mat->GetTextureCount(type); i++) 
	{
		...
		{
			if (loadedTextures[j].getPath().c_str() == (directory + str.C_Str()).c_str() || loadedTextures[j].getPath().c_str() == str.C_Str()) {
		...
		if (!skip)
		{
			const char* path = (pathIsRelative) ? (directory + str.C_Str()).c_str() : str.C_Str();
			Texture texture = Texture(path, textureType);
